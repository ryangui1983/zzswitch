...
export const channelFormSchema = z
  .object({
    ...
    disable_image_generation_tool: z.boolean().optional(), // OpenAI: strip image_generation tool to avoid 403
...
export function transformChannelToFormDefaults(
  ...
  let disableImageGenerationTool = false
  ...
  if (channel.settings) {
    try {
      ...
      disableImageGenerationTool = parsed.disable_image_generation_tool === true
  ...
  return {
    ...
    disable_image_generation_tool: disableImageGenerationTool,
...
function buildSettingsJSON(formData: ChannelFormValues): string {
  ...
  if (OPENAI_FIELD_PASSTHROUGH_TYPES.has(formData.type)) {
    ...
    settingsObj.disable_image_generation_tool = formData.disable_image_generation_tool === true
    ...
    if ('disable_image_generation_tool' in settingsObj) delete settingsObj.disable_image_generation_tool
