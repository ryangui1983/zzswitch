...
// parseStatusCodeRanges parses comma-separated status codes and ranges like "401,403,429,500-599".
func parseStatusCodeRanges(spec string) [][2]int {
	var ranges [][2]int
	for _, part := range strings.Split(spec, ",") {
		...
		if idx := strings.Index(part, "-"); idx > 0 {
			var lo, hi int
			if _, err := fmt.Sscanf(part[:idx], "%d", &lo); err != nil {
				continue
			}
			if _, err := fmt.Sscanf(part[idx+1:], "%d", &hi); err != nil {
				continue
			}
...
func (s ChannelSettings) ShouldRetrySameChannelByStatusCode(code int) bool {
	...
	spec := s.SchedulerPoolModeRetryStatusCodes
	if spec == "" {
		spec = "401,403,429"
	}
	for _, r := range parseStatusCodeRanges(spec) {
...
type ChannelOtherSettings struct {
	...
	DisableImageGenerationTool            bool                  `json:"disable_image_generation_tool,omitempty"` // 是否过滤请求中的 image_generation 工具（上游不支持生图时避免 403）
