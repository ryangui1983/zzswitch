...
func Relay(c *gin.Context, relayFormat types.RelayFormat) {
	...
	for attempts := 0; ; attempts++ {
		...
		processChannelError(c, *channelError, newAPIError, false, relayInfo)
	...
	if newAPIError != nil {
		if lastChannelError != nil {
			recordChannelErrorLog(c, *lastChannelError, newAPIError, relayInfo)
		}
		gopool.Go(func() {
	...
	for range workers {
		go func() {
			for job := range intermediateLogQueue {
				recordChannelErrorLogForUser(job.c, job.channelError, job.err, job.relayInfo, job.userId, job.username, true)
			}
		}()
	}
}

func enqueueIntermediateLog(c *gin.Context, channelError types.ChannelError, err *types.NewAPIError, relayInfo *relaycommon.RelayInfo) {
...
func processChannelError(c *gin.Context, channelError types.ChannelError, err *types.NewAPIError, recordErrorLog bool, relayInfo *relaycommon.RelayInfo) {
	logger.LogError(c, fmt.Sprintf("channel error (channel #%d, status code: %d): %s", channelError.ChannelId, err.StatusCode, common.LocalLogPreview(err.Error())))
	if relayInfo != nil && relayInfo.IsStream {
		handleChannelHealthOnChannelError(channelError, err.StatusCode, err.MaskSensitiveErrorWithStatusCode())
	}
	...
	if recordErrorLog {
		...
		enqueueIntermediateLog(c, channelError, err, relayInfo)
	}
}
...
func recordChannelErrorLog(c *gin.Context, channelError types.ChannelError, err *types.NewAPIError, relayInfo *relaycommon.RelayInfo) {
	recordChannelErrorLogForUser(c, channelError, err, relayInfo, c.GetInt("id"), c.GetString("username"), false)
}

func recordChannelErrorLogForUser(c *gin.Context, channelError types.ChannelError, err *types.NewAPIError, relayInfo *relaycommon.RelayInfo, userId int, username string, intermediateChannelError bool) {
	...
	other.SetPublic("intermediate_channel_error", intermediateChannelError)
		...
		if !taskErr.LocalError {
			processChannelError(c,
				*types.NewChannelError(channel.Id, channel.Type, channel.Name, channel.ChannelInfo.IsMultiKey,
				...
				types.NewOpenAIError(taskErr.Error, types.ErrorCodeBadResponseStatusCode, taskErr.StatusCode), true, relayInfo)
