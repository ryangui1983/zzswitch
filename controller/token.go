...
func AddToken(c *gin.Context) {
	...
	go model.RecordOperationAuditLog(cleanToken.UserId, c.GetInt("role"),
		fmt.Sprintf("created token '%s' with group '%s'", cleanToken.Name, cleanToken.Group),
		c.ClientIP(), "token.create",
		map[string]any{"name": cleanToken.Name, "group": cleanToken.Group},
		nil, nil, c)
...
func UpdateToken(c *gin.Context) {
	...
	if statusOnly == "" && oldGroup != cleanToken.Group {
		go model.RecordOperationAuditLog(userId, c.GetInt("role"),
			fmt.Sprintf("token '%s' group changed: '%s' -> '%s'", cleanToken.Name, oldGroup, cleanToken.Group),
			c.ClientIP(), "token.update_group",
			map[string]any{"name": cleanToken.Name, "token_id": cleanToken.Id, "old_group": oldGroup, "new_group": cleanToken.Group},
			nil, nil, c)
	}
	params["name"] = cleanToken.Name
	if statusOnly != "" {
